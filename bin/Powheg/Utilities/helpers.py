...
export LD_LIBRARY_PATH=`pwd`/lib/:`pwd`/lib64/:${LD_LIBRARY_PATH}",
          ...
          "dpapolVV" : "echo \"Adding Recola1.4.4 library\"\n \
if [ ! -f recola-collier-1.4.4.tar.gz ]; then\n \
  curl --output recola-collier-1.4.4.tar.gz https://recola.hepforge.org/downloads/?f=recola-collier-1.4.4.tar.gz || fail_exit \"Failed to get Recola tar ball \"\n \
fi\n \
...
mkdir obj-gfortran/proclib\n \
cd obj-gfortran/proclib\n \
cp ../../recola-collier-1.4.4/recola-1.4.4/librecola.so .\n \
cd ../..\n \
cp Makefile Makefile.orig\n \
cat Makefile.orig | sed -e \"s#RECOLALOCATION=.\+#RECOLALOCATION=$\(PWD\)/recola-collier-1.4.4/recola-1.4.4#g\" | sed -e \"s# real16.o##g\" | sed -e \"s#test#none#g\" | sed -e \"s#none_Suda#test_Suda#g\" > Makefile\n \
export LD_LIBRARY_PATH=`pwd`/lib/:`pwd`/lib64/:${LD_LIBRARY_PATH}",
